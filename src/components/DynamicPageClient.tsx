...
export default function DynamicPageClient({ slug, initialData }: { slug: string; initialData: PageData | null }) {
        ...
        async function loadData() {
            try {
                ...
                if (pages && pages.length > 0) {
                    setData(pages[0].attributes || pages[0]);
                }
            } catch (err) {
    ...
    if (loading) {
        return (
            <div style={{ background: "var(--bg-primary)", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <div className="tag-pill animate-pulse">Loading Content...</div>
            </div>
        );
    }
    ...
    if (!data) {
        return (
            <div style={{ background: "var(--bg-primary)", minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "1rem" }}>
                <h1 style={{ color: "var(--text-primary)" }}>Page Not Found</h1>
